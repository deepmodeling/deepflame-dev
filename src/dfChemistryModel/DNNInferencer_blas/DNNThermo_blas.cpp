...
#ifdef THERMO_USE_HBM
        munmap(output_buffer0_[i], output_buffer0_length[i]);  
#else
...
void DNNThermo_blas<DataType>::load_models(const std::string dir) {
    ...
    if(mpirank == 0 || !flag_mpi_init){
        ...
        std::ifstream fin(dir + "/model/parameter/setting.yaml");
        if (!fin) {
            std::cerr << "open setting file error , setting path : " << dir + "/model/parameter/setting.yaml" << std::endl;
    ...
    // load parameters
    for(int i = 0; i < model0_.size(); ++i){
        model0_[i]->load_parameters(dir+"/model/parameter", i);
    }
...
#ifdef THERMO_USE_HBM
        output_buffer0_length.push_back((batch_size_ * layers0_[i] * sizeof(DataType) + HBM_ALIGNED_SIZE - 1) / HBM_ALIGNED_SIZE * HBM_ALIGNED_SIZE);
        output_buffer0_.push_back((DataType *)mmap(NULL, (batch_size_ * layers0_[i] * sizeof(DataType) + HBM_ALIGNED_SIZE - 1) / HBM_ALIGNED_SIZE * HBM_ALIGNED_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0));
        printf("alloc hbm huge tlb memory %d Bytes done\n", (batch_size_ * layers0_[i] * sizeof(DataType) + HBM_ALIGNED_SIZE - 1) / HBM_ALIGNED_SIZE * HBM_ALIGNED_SIZE);
#else
...
//    printf("alloc hbm huge tlb memory done\n");
}
