...
DNNInferencer_blas<DataType>::~DNNInferencer_blas() {
    ...
    for(size_t i = 0; i < output_buffer_.size(); ++i){
        munmap(output_buffer_[i], output_buffer_length[i]);
    }
}
...
void DNNInferencer_blas<DataType>::load_models(const std::string dir){
    ...
    for(size_t i = 1; i < layers_.size() - 1; ++i){
        ...
        output_buffer_length.push_back((batch_size_ * layers_[i] * sizeof(DataType) + HBM_ALIGNED_SIZE - 1) / HBM_ALIGNED_SIZE * HBM_ALIGNED_SIZE);
        output_buffer_.push_back((DataType *)mmap(NULL, (batch_size_ * layers_[i] * sizeof(DataType) + HBM_ALIGNED_SIZE - 1) / HBM_ALIGNED_SIZE * HBM_ALIGNED_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0));
