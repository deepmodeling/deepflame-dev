...
void Foam::dfGAMGSolver::agglomerateMatrix
(
...
void Foam::dfGAMGSolver::agglomerateInterfaceCoefficients
(
    const label fineLevelIndex,
    const lduInterfacePtrsList& coarseMeshInterfaces,
    PtrList<lduInterfaceField>& coarsePrimInterfaces,
...
) const
{
    ...
    forAll(fineInterfaces, inti)
    {
        ...
        {
            const dfGAMGInterface& coarseInterface =
                refCast<const dfGAMGInterface>
                (
...
void Foam::dfGAMGSolver::gatherMatrices
(
...
) const
{
    if (debug)
    {
        Pout<< "dfGAMGSolver::gatherMatrices :"
...
void Foam::dfGAMGSolver::procAgglomerateMatrix
(
            ...
            forAll(bMap, procIntI)
            {
                ...
                if (allIntI != -1)
                {
                    ...
                    if (!allInterfaces.set(allIntI))
                    {
                        ...
                        if (proci == 0)
                        {
                            const processordfGAMGInterfaceField& procInt =
                                refCast
                                <
                                    const processordfGAMGInterfaceField
                                >
                                (
