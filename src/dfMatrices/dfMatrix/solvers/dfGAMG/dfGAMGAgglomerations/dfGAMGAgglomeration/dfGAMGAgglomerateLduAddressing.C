...
void Foam::dfGAMGAgglomeration::agglomerateLduAddressing
(
...
)
{
    ...
    forAll(fineInterfaces, inti)
    {
        if (fineInterfaces.set(inti))
        {
            ...
            patchFineToCoarse[inti] = refCast<const dfGAMGInterface>
            (
    ...
    (
        coarseInterfaces,
        lduPrimitiveMesh::nonBlockingSchedule<processordfGAMGInterface>
        (
            coarseInterfaces
        )
    );
...
void Foam::dfGAMGAgglomeration::procAgglomerateLduAddressing
(
...
void Foam::dfGAMGAgglomeration::procAgglomerateRestrictAddressing
(
    const label comm,
    const labelList& procIDs,
    const label levelIndex
...
void Foam::dfGAMGAgglomeration::combineLevels(const label curLevel)
{
    ...
    forAll(prevInterLevel, inti)
    {
        ...
        {
            dfGAMGInterface& prevInt = refCast<dfGAMGInterface>
            (
            ...
            const dfGAMGInterface& curInt = refCast<const dfGAMGInterface>
            (
                curInterLevel[inti]
            );
...
//void Foam::dfGAMGAgglomeration::gatherList
//(
//    const label comm,
//    const labelList& procIDs,
//
...
void Foam::dfGAMGAgglomeration::calculateRegionMaster
(
    const label comm,
    const labelList& procAgglomMap,
