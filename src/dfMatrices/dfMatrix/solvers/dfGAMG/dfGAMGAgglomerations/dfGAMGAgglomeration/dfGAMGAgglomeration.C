...
#include "dfGAMGProcAgglomeration.H"
#include "pairdfGAMGAgglomeration.H"
#include "IOmanip.H"
...
namespace Foam
{
    defineTypeNameAndDebug(dfGAMGAgglomeration, 0);
    defineRunTimeSelectionTable(dfGAMGAgglomeration, lduMesh);
    defineRunTimeSelectionTable(dfGAMGAgglomeration, lduMatrix);
    defineRunTimeSelectionTable(dfGAMGAgglomeration, geometry);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::dfGAMGAgglomeration::compactLevels(const label nCreatedLevels)
{
...
bool Foam::dfGAMGAgglomeration::continueAgglomerating
(
...
Foam::dfGAMGAgglomeration::dfGAMGAgglomeration
(
    const lduMesh& mesh,
    const dictionary& controlDict
)
:
    MeshObject<lduMesh, Foam::GeometricMeshObject, dfGAMGAgglomeration>(mesh),
    maxLevels_(
    ...
    (
      ...
      ? dfGAMGProcAgglomeration::New
        (
            controlDict.lookup("processorAgglomerator"),
            *this,
            controlDict
        )
      : autoPtr<dfGAMGProcAgglomeration>(nullptr)
    ),
...
const Foam::dfGAMGAgglomeration& Foam::dfGAMGAgglomeration::New
(
    const lduMesh& mesh,
    const dictionary& controlDict
)
{
    if
    (
        !mesh.thisDb().foundObject<dfGAMGAgglomeration>
        (
            dfGAMGAgglomeration::typeName
        )
    )
    {
        ...
        (
            controlDict,
            "geometricdfGAMGAgglomerationLibs",
        ...
        if (cstrIter == lduMeshConstructorTablePtr_->end())
        {
            FatalErrorInFunction
                << "Unknown dfGAMGAgglomeration type "
                << agglomeratorType << ".\n"
                << "Valid matrix dfGAMGAgglomeration types are :"
                << lduMatrixConstructorTablePtr_->sortedToc() << endl
                << "Valid geometric dfGAMGAgglomeration types are :"
    ...
    {
        return mesh.thisDb().lookupObject<dfGAMGAgglomeration>
        (
            dfGAMGAgglomeration::typeName
        );
    }
...
)
{
    ...
    )
    {
        ...
        (
            controlDict,
            "algebraicdfGAMGAgglomerationLibs",
...
Foam::autoPtr<Foam::dfGAMGAgglomeration> Foam::dfGAMGAgglomeration::New
(
    const lduMesh& mesh,
...
)
{
    ...
    return autoPtr<dfGAMGAgglomeration>
    (
        cstrIter()
        (
            mesh,
...
const Foam::lduMesh& Foam::dfGAMGAgglomeration::meshLevel
(
    const label i
) const
{
...
const Foam::lduInterfacePtrsList& Foam::dfGAMGAgglomeration::interfaceLevel
(
    const label i
) const
{
...
void Foam::dfGAMGAgglomeration::clearLevel(const label i)
{
    if (hasMeshLevel(i))
    {
...
const Foam::labelList& Foam::dfGAMGAgglomeration::procAgglomMap
(
    const label leveli
) const
{
...
const Foam::labelList& Foam::dfGAMGAgglomeration::agglomProcIDs
(
    const label leveli
) const
{
...
bool Foam::dfGAMGAgglomeration::hasProcMesh(const label leveli) const
{
    return procCommunicator_[leveli] != -1;
}


Foam::label Foam::dfGAMGAgglomeration::procCommunicator(const label leveli) const
{
    return procCommunicator_[leveli];
}


const Foam::labelList& Foam::dfGAMGAgglomeration::cellOffsets
(
    const label leveli
) const
{
...
const Foam::labelListList& Foam::dfGAMGAgglomeration::faceMap
(
    const label leveli
) const
{
...
const Foam::labelListList& Foam::dfGAMGAgglomeration::boundaryMap
(
    const label leveli
) const
{
...
const Foam::labelListListList& Foam::dfGAMGAgglomeration::boundaryFaceMap
(
    const label leveli
) const
{
...
bool Foam::dfGAMGAgglomeration::checkRestriction
(
